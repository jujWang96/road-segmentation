...
def layers(loaded_layers, num_classes):
    ...
    '''Decoder'''
    deconv_1 = tf.layers.conv2d_transpose(encoded_7, num_classes, 4, 8, "same", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01)) # padding?
    skip_1 = tf.add(deconv_1, encoded_2)

    deconv_3 = tf.layers.conv2d_transpose(skip_1, num_classes, 16, 4, "same", kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
